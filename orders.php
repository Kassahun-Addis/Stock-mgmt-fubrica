...
$productSql = "SELECT * FROM finished_good WHERE quantity >= 1 AND product_status = 1 ";
...
<script>
// Preload product options for reuse in JavaScript
let productOptions = `
    <?php echo $productOptions; ?>
`;
</script>

<script>document.querySelectorAll('input[name="rate[]"]').forEach(input => {
    input.addEventListener('input', function() {
        // Update the corresponding hidden field with the current value
        document.getElementById('rateValue' + this.id.replace('rate', '')).value = this.value;
    });
});
</script>
...
<div class="panel panel-default">
	<div class="panel-heading">
		...
		<?php } else if($_GET['o'] == 'manord') { ?>
			<i class="glyphicon glyphicon-edit"></i> Manage Order  <button class="btn btn-success" id="exportCsvBtn">
	...
	<div class="panel-body">
		...
		<?php if($_GET['o'] == 'add') { 
          ...
          <div class="form-group">
              <label for="sellsLocation" class="col-sm-2 control-label">Sells  Location</label>
              <div class="col-sm-10">
                    <select class="form-control" id="sellsLocation" name="sellsLocation">
                       <option value="">Select Sells Location</option>
                    <?php
			  ...
			  <div class="form-group">
			    <label for="fsNumber" class="col-sm-2 control-label">FS Number</label>
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="fsNumber" name="fsNumber" placeholder="FS Number" autocomplete="off" />
			    </div>
			  </div> <!--/form-group-->	
			   <div class="form-group">
                    <label for="tinNumber" class="col-sm-2 control-label">TIN Number</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="tinNumber" name="tinNumber" placeholder="TIN Number" autocomplete="off" />
                    </div>
                </div>
   <div class="form-group">
			  		...
			  		for($x = 1; $x < 2; $x++) { ?>
			  			<tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">			  				
			  				...
			  				<td>
			  					...
			  					<input type="text" name="serialNo[]" id="serialNo<?php echo $x; ?>" autocomplete="off" disabled="true" class="form-control" />			  					
			  					<input type="hidden" name="serialNoValue[]" id="serialNoValue<?php echo $x; ?>" autocomplete="off" class="form-control" />			  					
			  				</td>
			  			<td>
			  			    	<label>Rate</label>			  					
    <input type="text" name="rate[]" id="rate<?php echo $x; ?>" 
           autocomplete="off" 
           class="form-control"
           onfocus="capturePreviousRateValue(<?php echo $x; ?>)" 
           onchange="restrictRateChange(<?php echo $x; ?>)" />			  					
    <input type="hidden" name="rateValue[]" id="rateValue<?php echo $x; ?>" autocomplete="off" class="form-control" />			  					
			  				...
			  				<td>
			  					...
			  					<input type="text" name="purchase[]" id="purchase<?php echo $x; ?>" autocomplete="off" disabled="true" class="form-control" />			  					
			  					<input type="hidden" name="purchaseValue[]" id="purchaseValue<?php echo $x; ?>" autocomplete="off" class="form-control" />			  					
			  				</td>
			  			<td>
    ...
    <span id="quantity-error-<?php echo $x; ?>" style="color: red; font-weight: bold;"></span>
</td>
			  				...
			  				<td>
			  				 ...
			  				 <td>
        <input type="hidden" name="availableQuantity[]" id="availableQuantity<?php echo $x; ?>" />
        <!-- other fields -->
    </td>
			  			</tr>
		  			<?php
			  ...
			  <div class="col-md-6">
				  ...
				  <div class="form-group">
					...
					<div class="form-group">
    <label class="col-sm-3 control-label">Apply Withhold</label>
    <div class="col-sm-9">
        <!-- <input type="checkbox" id="applyWithhold" /> -->
		<input type="checkbox" id="applyWithhold"/>
        <label for="applyWithhold">Yes</label>
    </div>
</div>

<div id="withholdContainer" class="form-group">
    <label for="withhold" class="col-sm-3 control-label">Withhold</label>
    <div class="col-sm-9">
        <input type="text" class="form-control" id="withhold" name="withhold" readonly="true" />
    </div>
</div>
				  </div> <!--/form-group-->	
				  <div class="form-group">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-9">
      <select class="form-control" name="paymentType" id="paymentType" onchange="togglePaymentDetails()">
        <option value="">~~SELECT~~</option>
...
<div class="form-group">
  <div class="col-sm-12">
    <div class="div-action pull-right" style="padding-bottom:10px;">
      <button class="btn btn-default button1" data-toggle="modal" id="addBankModalBtn" data-target="#addBankModal">
        <i class="glyphicon glyphicon-plus-sign"></i> Add Bank
      </button>
    </div>
  </div>
</div>
...
<div class="form-group">
  ...
  <div id="transactionDetails" class="form-group" style="display: none;">
		...
		// /else manage order
  		...
  		<form class="form-horizontal" method="POST" action="php_action/editOrder.php" id="editOrderForm">
			...
			orders.vat, orders.total_amount, orders.discount, orders.grand_total, orders.paid, orders.due, orders.payment_type,
			 orders.payment_status, orders.fs_no FROM orders 	
			  ...
			  <div class="form-group">
				...
				<label for="fsNumber" class="col-sm-2 control-label">FS Number</label>
				<div class="col-sm-10">
					<input type="text" class="form-control" id="fsNumber" name="fsNumber" placeholder="FS Number" autocomplete="off" value="<?php echo $data[13] ?>" />
				</div>
				</div> <!--/form-group-->			  
			  ...
			  <table class="table" id="productTable">
			  	...
			  	<tbody>
			  		...
			  		$orderItemSql = "SELECT order_item.order_item_id, order_item.order_id, order_item.product_id, order_item.quantity, order_item.rate, order_item.total, order_item.serial_no,
					 order_item.purchase FROM order_item WHERE order_item.order_id = {$orderId}";
			  		...
			  		while($orderItemData = $orderItemResult->fetch_array()) { 
			  			...
			  			<tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">			  				
			  				...
			  				<td style="padding-left:20px;">			  					
			  					<input type="text" name="serialNo[]" id="serialNo<?php echo $x; ?>" autocomplete="off" disabled="true" class="form-control" value="<?php echo $orderItemData['serial_no']; ?>" />			  					
			  					<input type="hidden" name="serialNoValue[]" id="serialNoValue<?php echo $x; ?>" autocomplete="off" class="form-control" value="<?php echo $orderItemData['serial_no']; ?>" />			  					
			  				</td>
			  		     <td style="padding-left:20px;">			  					
			  					...
			  					<input type="text" name="purchase[]" id="purchase<?php echo $x; ?>" autocomplete="off" disabled="true" class="form-control" value="<?php echo $orderItemData['purchase']; ?>" />			  					
			  					<input type="hidden" name="purchaseValue[]" id="purchaseValue<?php echo $x; ?>" autocomplete="off" class="form-control" value="<?php echo $orderItemData['purchase']; ?>" />			  					
			  				</td>
			  				<td style="padding-left:20px;">
			  					<div class="form-group">
			  					<input type="number" name="quantity[]" id="quantity<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)"  onChange="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control" min="1" value="<?php echo $orderItemData['quantity']; ?>" />
			  					</div>
			  				</td>
			  				<td style="padding-left:20px;">			  					
			  ...
			  <div class="col-md-6">
			  	<div class="form-group">
				    <label for="subTotal" class="col-sm-3 control-label">Purchase Total</label>
				    <div class="col-sm-9">
...
<script>
    document.getElementById('exportCsvBtn').addEventListener('click', function() {
        window.location.href = 'php_action/exportOrderToCsv.php';  // Direct the button click to the PHP script
    });
</script>
